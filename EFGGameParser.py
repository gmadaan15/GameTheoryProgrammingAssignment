    ...
    def __str__(self):
        ...
        for child in self.children:
            if (isinstance(child[1], Personal_node)):
                children.append("({}-{}-{})".format(child[0], child[1].owner_player, child[1].info_num))
            else:
                children.append("({}-{}-{})".format(child[0], child[1].node_name, child[1].payoffs))
        children = ', '.join(children)

        return """PERSONAL_NODE(name: '{}', owner: {}, info_set: ({}, '{}'), actions: {}, outcome: ({}, '{}'), payoffs: {},
                children: ({}))""".format(
            self.node_name, self.owner_player, self.info_num, self.info_name, self.action_names, self.outcome, self.outcome_name, self.payoffs, children
        )
...
class EfgGameParser(object):
    ...
    def parse_efg_file(cls, filename):
        ...
        # lets match for the complete file, payload may be multiline
